# nearly direct copy of https://raw.githubusercontent.com/NixOS/nixpkgs/refs/heads/nixos-24.11/nixos/modules/services/matrix/mjolnir.nix
# modified to expose a package option.
...
let
  ...
  configFiles = [
    "${cfg.package}/libexec/mjolnir/deps/mjolnir/config/default.yaml"
    moduleConfigFile
  ];
...
in
{
  options.services.mjolnir = {
    ...
    package = lib.mkOption {
      ...
      defaultText = "pkgs.mjolnir";
      description = "The package to use for mjolnir";
    };
  ...
  config = lib.mkIf config.services.mjolnir.enable {
    ...
    systemd.services.mjolnir = {
      ...
      serviceConfig = {
        ExecStart = ''${cfg.package}/bin/mjolnir --mjolnir-config ./config/default.yaml'';
